...
public function getDisponibilidad(Request $request)
{
    ...
    try {
        ...
        $diaSemana = date('l', strtotime($fecha)); // Obtiene el día en inglés

        // Si es domingo, directamente devolver un mensaje de no disponibilidad
        if ($diaSemana === 'Sunday') {
            return response()->json(['error' => 'No hay horarios disponibles los domingos.'], 200);
        }

        // Llamada al procedimiento almacenado
        $horarios = DB::connection('sqlsrv_proveedores')->select(
            "SET NOCOUNT ON; EXEC sp_horarios_disponibles @sucursal_id = ?, @fecha = ?",
            [$sucursal_id, $fecha]
        );

        // Si el array de horarios está vacío, la sucursal no trabaja ese día o no hay disponibilidad
        if (empty($horarios)) {
            return response()->json(['error' => 'No hay horarios disponibles para la fecha seleccionada.'], 200);
        }
