...
@section('content')
    ...
    <link rel="stylesheet" href="{{ asset('assets/css/rol/proveedor/cita.css') }}">
    <!-- Bootstrap JS Bundle (incluye Popper) -->
    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.2/dist/js/bootstrap.bundle.min.js" crossorigin="anonymous"></script>

    <!-- Session Rol -->
    @if (!session()->has('Proveedor') && !(session('Usuario') && in_array(session('Usuario.IdRol'), [1, 3, 5])))
        <script>
    ...
    <div class="container">
        ...
        <form id="citaForm" method="POST" action="{{ route('proveedor.citas.store') }}">
            @csrf
                        ...
                        @foreach ($campos as $label => $id)
                            <div class="form-check mb-2">
                                ...
                                <label class="form-check-label"
                                    for="check_{{ $id }}">{{ $label }}</label>
                                <input type="text" class="form-control mt-2 d-none campo-input"
                                    id="input_{{ $id }}"
                                    placeholder="Ingrese nuevo  para {{ $label }}">
                            </div>
                        @endforeach
                    </form>
                </div>
    ...
    <script>
        const PRECIO_POR_CAJA = {!! json_encode($precioCaja) !!};
    </script>
